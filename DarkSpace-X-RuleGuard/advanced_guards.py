# advanced_guards.py — eight survey-derived blue-team techniques for agentic LLM UIs
"""
...
_BENIGN_BASELINE: list[str] = [
    ...
    # --- Aligned to benign-enterprise controls in literature/survey-informed fixture ---
...
def ruleguard_base_stack(prompt: str, history: list[str] | None = None) -> dict:
    ...
    return {
        ...
        "metadata": {
            **base.get("metadata", {}),
            "technique": "ruleguard_semantic_embedding_stack",
            "semantic_similarity": sem_score,
            "semantic_safe": sem_safe,
            "total_layers": 21,
        },
...
# Default path: Hugging Face `transformers` with a small model for reproducible CI.
...
# Verbatim production judge template (do not edit casually for model swaps).
...
def llm_as_judge_check(prompt: str, hypothetical_output: str = "") -> tuple[bool, float, str]:
    ...
    spec = config.get_active_judge_spec()
    # === Loud status logging (visibility for integration debugging) ===
...
def _mini_templar_thread_inputs(
    ...
    """Map production thread (last user + role-tagged history) to Mini Templar classify_text inputs."""
...
def adversarial_framing_block(prompt: str) -> bool:
    """
    Named attack framings from survey-informed literature that evade generic rebuff
    (e.g. translation-smuggling, fake tool traces). Tuned for precision on enterprise benigns.
    """
    if not getattr(config, "ADVERSARIAL_FRAME_BLOCK_LAYER", True):
        return False
    pl = prompt.lower()
...
def ruleguard_full_stack_with_judge(prompt: str, history: list[str] | None = None) -> dict:
    """24 layers max adversarial sensitivity + layer 25 (3-shot classifier) + BIPIA + hierarchy + 405B ensemble."""
    base = ruleguard_base_stack(prompt, history)
    ...
    framing_block = adversarial_framing_block(prompt)
    ...
    ensemble_voting = {
        ...
        "blockers": {
            ...
            "adversarial_framing": framing_block,
