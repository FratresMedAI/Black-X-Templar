...
def _env_bool_with_legacy(primary: str, legacy: str, default: bool) -> bool:
    """Prefer `primary`; if unset, fall back to `legacy` (older deployments)."""
    raw_p = os.environ.get(primary, "").strip().lower()
    if raw_p in ("1", "true", "yes", "on"):
        return True
    if raw_p in ("0", "false", "no", "off"):
        return False
...
def validate_mirror_startup_smoke() -> None:
    ...
    if not results or not all(r["correct"] for r in results):
        ...
        raise ValueError("validate_mirror_startup_smoke: neural_mirror extended smoke failed" + detail)


# Blue-team extensions (integration canary and smoothing)
CANARY_TOKEN = "RULEGUARD_MIRROR_INTEGRITY_42_DO_NOT_REMOVE"
...
#      python test_harness.py --judge llama-3.1-405b-instruct-4bit --dataset path/to/fixture.jsonl
...
# ── Embedding / semantic gate defaults (tunable via DARKSPACE_* env) ─
# Calibrated against survey-informed curated fixtures and benign-enterprise prompts.
# Override: DARKSPACE_SEMANTIC_THRESHOLD, DARKSPACE_SECONDARY_SEMANTIC_FLOOR, etc.
...
# High-precision blocks for named jailbreak frames from survey literature (low benign FP).
ADVERSARIAL_FRAME_BLOCK_LAYER = _env_bool_with_legacy(
    "DARKSPACE_ADVERSARIAL_FRAME_BLOCK",
    "DARKSPACE_GRAY_SWAN_FRAME_BLOCK",
    True,
)
