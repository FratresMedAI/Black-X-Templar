...
Hot path must stay pure (no DB, no network) for classifier latency and deployment policy.
...
_INJECTION_PATTERNS: list[tuple[str, float]] = [
    ...
    # High-precision survey / marketplace framings (see adversarial taxonomy rows in curated fixtures)
    ...
    # Agent-UI abuse shapes (high precision vs. typical enterprise benign in curated fixtures).
    ...
    # Multi-agent / orchestration abuse framings (survey-taxonomy aligned)
    ...
    # Agent harness follow-ons: synthetic maintenance, fake sessions, tool-shaped users, handoffs
    ...
    # Synthetic-order / session-xml / voucher-exfil abuse (high precision on curated harmful fixtures)
    ...
    # Instruction-override lures: benign-seeming topics paired with policy overrides (fixture-checked)
    ...
    # Trigram / handoff smuggling (curated harmful vs benign separation)
    ...
    # Low-signal orchestration misses (precision-tuned on labeled fixtures)
    ...
    # Orchestrator smuggling, separator tricks, compensation lures (fixture-checked benigns)
    ...
    # Encoding smuggling + orchestration abuse (survey-aligned hardening pass)
    (r"[\u2800-\u28FF]{24,}", 8.9),
    (r"[\u16A0-\u16FF]{16,}", 8.7),
    ...
    # --- 2026-04-27+ rule wave (precision-tight strings; survey / red-team corpus alignment) ---
...
# Curated harmful-vs-benign fixtures: high precision per phrase on held-out enterprise benigns.
_FULLTHREAD_PATTERNS: list[tuple[str, float]] = [
    ...
    # ChatML / role-boundary hints (common jailbreak transports)
    (r"<\|im_start\|>\s*system", 9.0),
    (r"<\|im_end\|>", 8.5),
    # Voucher / delegation exfil variants (fixture-checked)
    ...
    # Echo / split-turn overlap with multi-turn full-thread scan
    (r"(?i)get\s+my\s+store\s+vouchers", 8.8),
    (r"(?is)<\s*system_message\s*>", 8.9),
    (r"(?i)backup\s+account\s+id\s+customer_", 8.9),
    (r"(?i)tell\s+the\s+order\s+man\s+precisely", 8.9),
