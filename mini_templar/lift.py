...
_MXT_LIFT_RAW: list[tuple[str, float]] = [
    ...
    (r"(?i)instruction-override-harmful-w2", 9.1),
    (
        r"(?is)(instruction|system|developer|policy)\s*[-_ ]?override[\s\S]{0,400}"
        r"(harmful|unsafe|blocked|bypass|ignore|do\s+not\s+refuse)",
        9.1,
    ),
    (
        r"(?is)(ignore|override|disregard)[\s\S]{0,180}"
        r"(safety|policy|guardrail|moderation|refusal)[\s\S]{0,320}"
        r"(harmful|unsafe|restricted|blocked|disallowed)",
        9.1,
    ),
