...
@pytest.fixture
def real_2026_rows(repo_root: Path) -> list[dict]:
    p = repo_root / "tests" / "real_2026_training_dataset.jsonl"
    if not p.is_file():
...
def stress_ood_rows(repo_root: Path) -> list[dict]:
    p = repo_root / "tests" / "stress_ood_2026.jsonl"
    if not p.is_file():
        pytest.skip(f"missing {p}")
    return load_jsonl(p)
...
def stress_agentic(repo_root: Path) -> list[dict]:
    p = repo_root / "tests" / "stress_agentic_min.json"
    if not p.is_file():
        pytest.skip(f"missing {p}")
