...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="DARKSPACE OSINT Expert — NVD CVE feed correlator"
    )
    parser.add_argument("--loop", action="store_true",
                        help="Run continuously (default: run once and exit)")
    parser.add_argument("--days", type=int, default=1,
                        help="How many days back to query (default: 1)")
    parser.add_argument("--verbose", "-v", action="store_true")
    parser.add_argument("--offline-only", action="store_true",
                        help="Disable external network fetches and run from local DB only")
    args = parser.parse_args()

    offline_mode = args.offline_only or config.OFFLINE_ONLY

    if args.loop:
        run_loop(days_back=args.days, offline_only=offline_mode)
    else:
        run_once(days_back=args.days, verbose=args.verbose, offline_only=offline_mode)
