...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="DARKSPACE Vault Guardian — alert-driven mock key rotation"
    )
    parser.add_argument("--once", action="store_true",
                        help="Run one check and exit (default: continuous loop)")
    parser.add_argument("--interval", type=int,
                        default=config.VAULT_CHECK_INTERVAL_SECONDS,
                        help=f"Check interval in seconds "
                             f"(default: {config.VAULT_CHECK_INTERVAL_SECONDS})")
