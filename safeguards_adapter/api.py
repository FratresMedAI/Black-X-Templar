"""
HTTP adapter for Gray Swan Safeguards (provisional contract — see SUBMISSION_CONTRACT.md).
"""
...
app = FastAPI(title="DARKSPACE Safeguards Adapter", version=__version__)


class ConversationTurn(BaseModel):
    """Gray Swan Safeguards recommended input shape."""
...
class ClassifyRequest(BaseModel):
    ...
    conversation: list[ConversationTurn] | None = Field(
        default=None,
        description="Gray Swan format: multi-turn messages to classify",
    )
    text: str = Field(
        default="",
...
class ClassifyResponse(BaseModel):
    """Gray Swan recommended: violation + confidence; legacy fields kept for debugging."""
...
def _violation_and_confidence(c: Classification) -> tuple[bool, float]:
    """
    Gray Swan: optional confidence-only path treats confidence > 0.5 as violation.
    We always set violation explicitly; for allows we cap confidence at 0.5 so
    confidence-only fallback never fires incorrectly.
    """
    if c.decision == "block":
...
def _threshold() -> float:
    # Align with classify_text default used in local Orbit evals; override via env for Arena.
    raw = os.environ.get("SAFEGUARDS_BLOCK_THRESHOLD", "5.5")
    try:
        return float(raw)
    except ValueError:
        return 5.5
